...
-------------------------------------------------------------------------
|									|
|	Aptina MT9M021 (1.2MP) CMOS image sensor linux driver release	|
|									|
-------------------------------------------------------------------------
    Release notes for the Aptina MT9M021 image sensor linux driver.
...
MT9M021 Driver
--------------
    The MT9M021 driver is a Video for Linux 2 (V4L2) driver that is compiled
...
------------------------
    ...
    - Aptina Beagleboard-XM Adapter/Revision 0 and MT9M021 1.2MP CMOS sensor.


DRIVER SOURCE CODE FILES
------------------------
    Driver files and directory locations are listed below:
    mt9m021.c, Makefile, and Kconfig are located at:
        kernel-3.1.2/drivers/media/video

    mt9m021.h is located at:
...
--------------------------------------
    Copy MT9M021 related files into the kernel directories:
        $cp your_mt9m021_driver_directory/board-omap3beagle.c		./arch/arm/mach-omap2
        $cp your_mt9m021_driver_directory/board-omap3beagle-camera.c	./arch/arm/mach-omap2
        $cp your_mt9m021_driver_directory/mt9m021.c			./drivers/media/video
        $cp your_mt9m021_driver_directory/Makefile			./drivers/media/video
        $cp your_mt9m021_driver_directory/Kconfig			./drivers/media/video
        $cp your_mt9m021_driver_directory/mt9m021.h			./include/media
    ...
    In menuconfig, enable the MT9M021 driver. The MT9M021 driver configuration is at the
    following location:
        "Device Drivers"-->"Multimedia support"-->"Video capture adapters"-->"Encoders/decoders and other helper chips" 

    Select "Aptina MT9M021 support"
	<*> Aptina MT9M021 support

    Note: other image sensors (such as the MT9V011 or MT9P015) should be de-selected.

    Enable/Disable MT9M021 driver compilation options that are located in the mt9m021.c file:
        MT9M021_DEBUG - Define this to enable debug print support
            Undefine this for a driver release. 
        MT9M021_I2C_DEBUG - Define this to enable i2c register writes debug print support
            Undefine this for a driver release. 
    ...
    Copy the driver to the EXT3 partition if the mt9m021 driver is configured as a loadable module
        $cp drivers/media/video/mt9m021.ko  /media/Anstrom/home/root
...
MT9M021 SUPPORTED OUTPUT FRAME FORMATS
------------------------------
  RGB
