...
-----------------------------------------------------------------------
|                                                                     |
|Aptina MT9P006 (5-meg pixel ) image sensor linux driver release   |
|                                                                     |
-----------------------------------------------------------------------
    Release notes for the Aptina MT9P006 image sensor linux driver.
...
MT9P006 Driver
--------------
    The MT9P006 driver is a Video for Linux 2 (V4L2) driver that is compiled
...
------------------------
    ...
    - Aptina Beagleboard-XM Adapter/Revision 0 and MT9P006 5-megapixel sensor.


DRIVER SOURCE CODE FILES
------------------------
    Driver files and directory locations are listed below:
    mt9p006.c, Makefile, and Kconfig are located at:
        kernel-2.6.39/drivers/media/video

    mt9p006.h and v4l2-chip-ident.h are located at:
...
--------------------------------------
    Copy MT9P006 related files into the kernel directories:
        $cp your_mt9p006_driver_directory/board-omap3beagle.c  ./arch/arm/mach-omap2
        $cp your_mt9p006_driver_directory/board-omap3beagle-camera.c  ./arch/arm/mach-omap2
        $cp your_mt9p006_driver_directory/mt9p006.c            ./drivers/media/video
        $cp your_mt9p006_driver_directory/Makefile             ./drivers/media/video
        $cp your_mt9p006_driver_directory/Kconfig              ./drivers/media/video
        $cp your_mt9p006_driver_directory/mt9p006.h            ./include/media
        $cp your_mt9p006_driver_directory/v4l2-chip-ident.h    ./include/media
    ...
    In menuconfig, enable the MT9P006 driver. The MT9P006 driver configuration is at the
    following location:
        "Device Drivers"-->"Multimedia support"-->"Video capture adapters"-->"Encoders/decoders and other helper chips" 

    Select "Aptina A-51HD+ 5 MP Sensor Support" as
        <*> Aptina A-51HD+ 5 MP Sensor Support"

    Note: other image sensors (such as the MT9V011 or MT9P012) should be de-selected.

    Enable/Disable MT9P006 driver compilation options that are located in the mt9p006.c file:
        MT9P006_DEBUG - Define this to enable debug printk support
    ...
    Copy the driver to the EXT3 partition if the mt9p006 driver is configured as a loadable module
        $cp drivers/media/video/mt9p006.ko  /media/Anstrom/home/root
...
MT9P006 SUPPORTED OUTPUT FRAME SIZES
------------------------------------
...
MT9P006 SUPPORTED OUTPUT FRAME FORMATS
------------------------------
  SRGB
