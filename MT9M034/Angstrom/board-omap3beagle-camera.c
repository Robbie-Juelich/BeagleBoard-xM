...
#ifdef CONFIG_VIDEO_MT9P111
#define MT9P111_RESET_GPIO      98
#define MT9P111_XCLK            ISP_XCLK_A
#define MT9P111_EXT_FREQ        24000000
#define MT9P111_I2C_ADDR	0x3C /* (0x78 >> 1) */	
//#define MT9P111_I2C_ADDR	0x3D /* (0x7A >> 1) */	

static int beagle_cam_set_xclk(struct v4l2_subdev *subdev, int hz)
{
        struct isp_device *isp = v4l2_dev_to_isp_device(subdev->v4l2_dev);

        return isp->platform_cb.set_xclk(isp, hz, MT9P111_XCLK);
}

static int beagle_cam_reset(struct v4l2_subdev *subdev, int active)
{
        /* Set RESET_BAR to !active */
        gpio_set_value(MT9P111_RESET_GPIO, !active);
...
static struct mt9p111_platform_data beagle_mt9p111_platform_data = {
	.set_xclk       = beagle_cam_set_xclk,
	.reset          = beagle_cam_reset,
	.ext_freq       = MT9P111_EXT_FREQ,
	.target_freq    = 48000000,
};

static struct i2c_board_info mt9p111_camera_i2c_device = {
        I2C_BOARD_INFO("mt9p111", MT9P111_I2C_ADDR),
        .platform_data = &beagle_mt9p111_platform_data,
};

static struct isp_subdev_i2c_board_info mt9p111_camera_subdevs[] = {
        {
                .board_info = &mt9p111_camera_i2c_device,
                .i2c_adapter_id = 2,
        },
        { NULL, 0, },
};
...
static struct isp_v4l2_subdevs_group beagle_camera_subdevs[] = {
        {
                .subdevs = mt9p111_camera_subdevs,
                .interface = ISP_INTERFACE_PARALLEL,
                .bus = {
                        .parallel = {
                                .data_lane_shift = 2,
...
/* MT9V113 Support */
#ifdef CONFIG_VIDEO_MT9V113
#define MT9V113_RESET_GPIO	98
#define MT9V113_XCLK		ISP_XCLK_A
#define MT9V113_EXT_FREQ	14400000

static int beagle_cam_set_xclk(struct v4l2_subdev *subdev, int hz)
{
	struct isp_device *isp = v4l2_dev_to_isp_device(subdev->v4l2_dev);

	return isp->platform_cb.set_xclk(isp, hz, MT9V113_XCLK);
}

static int beagle_cam_reset(struct v4l2_subdev *subdev, int active)
{
	/* Set RESET_BAR to !active */
	gpio_set_value(MT9V113_RESET_GPIO, !active);
...
static struct mt9v113_platform_data beagle_mt9v113_platform_data = {
	.set_xclk	= beagle_cam_set_xclk,
	.reset		= beagle_cam_reset,
	.ext_freq	= MT9V113_EXT_FREQ,
	.target_freq	= 28800000,
};

static struct i2c_board_info mt9v113_camera_i2c_device = {
	I2C_BOARD_INFO("mt9v113", MT9V113_I2C_ADDR),
	.platform_data = &beagle_mt9v113_platform_data,
};

static struct isp_subdev_i2c_board_info mt9v113_camera_subdevs[] = {
	{
		.board_info = &mt9v113_camera_i2c_device,
		.i2c_adapter_id = 2,
	},
	{ NULL, 0, },
};
...
static struct isp_v4l2_subdevs_group beagle_camera_subdevs[] = {
	{
		.subdevs = mt9v113_camera_subdevs,
		.interface = ISP_INTERFACE_PARALLEL,
		.bus = {
			.parallel = {
				.data_lane_shift = 2,
...
#ifdef CONFIG_VIDEO_MT9M021
#define MT9M021_RESET_GPIO	98
#define MT9M021_XCLK		ISP_XCLK_A
...
static int beagle_cam_set_xclk(struct v4l2_subdev *subdev, int hz)
{
	...
	return isp->platform_cb.set_xclk(isp, hz, MT9M021_XCLK);
}

static int beagle_cam_reset(struct v4l2_subdev *subdev, int active)
{
	/* Set RESET_BAR to !active */
	gpio_set_value(MT9M021_RESET_GPIO, !active);

	return 0;
}

static struct mt9m021_platform_data beagle_mt9m021_platform_data = {
	.set_xclk	= beagle_cam_set_xclk,
	.reset		= beagle_cam_reset,
...
static struct i2c_board_info mt9m021_camera_i2c_device = {
	I2C_BOARD_INFO("mt9m021", MT9M021_I2C_ADDR),
	.platform_data = &beagle_mt9m021_platform_data,
};

static struct isp_subdev_i2c_board_info mt9m021_camera_subdevs[] = {
	{
		.board_info = &mt9m021_camera_i2c_device,
		.i2c_adapter_id = 2,
	},
	{ NULL, 0, },
};
...
static struct isp_v4l2_subdevs_group beagle_camera_subdevs[] = {
	{
		.subdevs = mt9m021_camera_subdevs,
		.interface = ISP_INTERFACE_PARALLEL,
		.bus = {
			.parallel = {
				.data_lane_shift = 0,
...
#ifdef CONFIG_VIDEO_MT9M034
#define MT9M034_RESET_GPIO	98
#define MT9M034_XCLK		ISP_XCLK_A
#define MT9M034_EXT_FREQ	24000000

static int beagle_cam_set_xclk(struct v4l2_subdev *subdev, int hz)
{
	struct isp_device *isp = v4l2_dev_to_isp_device(subdev->v4l2_dev);

	return isp->platform_cb.set_xclk(isp, hz, MT9M034_XCLK);
}

static int beagle_cam_reset(struct v4l2_subdev *subdev, int active)
{
	/* Set RESET_BAR to !active */
	gpio_set_value(MT9M034_RESET_GPIO, !active);

	return 0;
}

static struct mt9m034_platform_data beagle_mt9m034_platform_data = {
	.set_xclk	= beagle_cam_set_xclk,
	.reset		= beagle_cam_reset,
	.ext_freq	= MT9M034_EXT_FREQ,
	.target_freq	= 48000000,
	.version        = MT9M034_COLOR_VERSION,
};

static struct i2c_board_info mt9m034_camera_i2c_device = {
	I2C_BOARD_INFO("mt9m034", MT9M034_I2C_ADDR),
	.platform_data = &beagle_mt9m034_platform_data,
};

static struct isp_subdev_i2c_board_info mt9m034_camera_subdevs[] = {
	{
		.board_info = &mt9m034_camera_i2c_device,
		.i2c_adapter_id = 2,
	},
	{ NULL, 0, },
};
...
static struct isp_v4l2_subdevs_group beagle_camera_subdevs[] = {
	{
		.subdevs = mt9m034_camera_subdevs,
		.interface = ISP_INTERFACE_PARALLEL,
		.bus = {
			.parallel = {
				.data_lane_shift = 0,
...
//AP0100 Support
#ifdef CONFIG_VIDEO_AP0100
#define AP0100_RESET_GPIO	98
#define AP0100_XCLK		ISP_XCLK_A
#define AP0100_EXT_FREQ		24000000

static int beagle_cam_set_xclk(struct v4l2_subdev *subdev, int hz)
{
	struct isp_device *isp = v4l2_dev_to_isp_device(subdev->v4l2_dev);

	return isp->platform_cb.set_xclk(isp, hz, AP0100_XCLK);
}

static int beagle_cam_reset(struct v4l2_subdev *subdev, int active)
{
        /* Set RESET_BAR to !active */
        gpio_set_value(AP0100_RESET_GPIO, !active);

        return 0;
}

static struct ap0100_platform_data beagle_ap0100_platform_data = {
        .set_xclk       = beagle_cam_set_xclk,
        .reset          = beagle_cam_reset,
        .ext_freq       = AP0100_EXT_FREQ,
        .target_freq    = 74000000,
};

static struct i2c_board_info ap0100_camera_i2c_device = {
        I2C_BOARD_INFO("ap0100", AP0100_I2C_ADDR),
        .platform_data = &beagle_ap0100_platform_data,
};

static struct isp_subdev_i2c_board_info ap0100_camera_subdevs[] = {
        {
                .board_info = &ap0100_camera_i2c_device,
                .i2c_adapter_id = 2,
        },
        { NULL, 0, },
};
...
#define AR0130_EXT_FREQ        24000000
...
#endif
#ifdef CONFIG_VIDEO_MT9M034
	gpio_request(MT9M034_RESET_GPIO, "cam_rst");
	gpio_direction_output(MT9M034_RESET_GPIO, 0);
#endif
