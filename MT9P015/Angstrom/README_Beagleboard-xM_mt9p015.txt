...
---------------------------------------------------
    ...
    esolutions such as VGA(640x480) or 720p(1280x720):
        $mplayer tv:// -tv driver=v4l2:width=640:height=480:device=/dev/video0:fps=30 -vo x11
        $mplayer tv:// -tv driver=v4l2:width=1280:height=720:device=/dev/video0:fps=30 -vo x11
...
-------------------------------------------------
    ...
    To access sensor registers, navigate to the following system directory:
        $cd /sys/devices/platform/i2c_omap.2/i2c-2/2-0036/

    To adjust the sensor exposure time, write to the exposure_val file.  The exposure range is 
    2000-27000.  The following example writes 33000 to the exposure_val file:
        $echo 27000 > exposure_val
    ...
    has a range from from 0-159. The following example writes 64 to the gain_val file:
        $echo 64 > gain_val
    ...
    To write to the register at address 0x3028 with value 0x1DDD:
        $echo 0x3028 > adv_reg_addr
        $echo 0x1DDD > adv_reg_val

    To read 8-bit value from the register at address 0x3028:
        $echo 0x3028 > basic_reg_addr
        $cat basic_reg_val

    To write to the register at address 0x3028 with value 0x08:
        $echo 0x3028 > basic_reg_addr
        $echo 0x08 > basic_reg_val


LIMITATIONS
-----------
    Preview resolutions supported from driver are VGA, 720p and 1080p.
    Resolutions lower than VGA can be supported if supported by isp-resizer.
    AE and AWB are disabled by default. AF is not supported.
    Resolutions till 720p supports both frame rates 30fps and 13fps.
    However, only frame rate 30fps is enabled for these resolutions.
    For all other resolutions, supported frame rate is 13fps only.
    Only a subset of V4L2 commands are supported.
